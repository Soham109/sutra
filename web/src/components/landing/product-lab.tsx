...
const MODES: Record<Mode, {
  ...
  plan: {
    ...
    facts: ['Live availability signals', 'Real places, never invented', 'Every score explained'],
  },
  link: {
    nav: 'Paste the link', number: '02',
    title: 'Bring any checkout into the room.',
    copy: 'Paste a product, ticket, stay or cart URL. Sutra reads the merchant’s own structured data, then lets the group claim items and shared fees.',
  ...
  bill: {
    ...
    copy: 'On-device OCR reads first. A tiny vision model can provide a second opinion, but deterministic arithmetic still has to reconcile every line to the printed total.',
    action: 'Scan a bill', href: '/app/bill',
    facts: ['Local OCR first', 'Nothing invented to balance', 'Explicit at-venue acceptance'],
  },
  circle: {
    nav: 'Bring the circle', number: '04',
    title: 'The people you keep planning with.',
    copy: 'Save the crew once, reuse it everywhere, and see private reliability facts derived from real Sutra events—not a public social score.',
    action: 'Open circles', href: '/app/circles',
    facts: ['Reusable people', 'Private reliability', 'Live exposure view'],
  },
}
