...
from nanda_town_prava import Principal, PravaMandates, reset_shared_state  # noqa: E402
...
async def run_simulated_scene() -> None:
    ...
    show_members(view)
    check("four mandate sessions minted, none approved yet", group.status == "collecting",
          group.status)
    check("every principal's cap is their own — nobody's cap depends on anybody else's",
    ...
    check("the group is still open after Dev's decline (quorum(2) is still reachable)",
          mid["status"] == "collecting", mid["status"])
    ...
    check("group committed despite a mid-flight decline", final["status"] == "committed",
          final["status"])
    check("Dev was never charged", m["Dev"]["charged_amount"] == 0)
    check("Soham and Arsh were each capped at the number they consented to, not the "
    ...
    say(f"Maya's backstop_cap was {BACKSTOP_CAP}; the shortfall actually drawn from her "
        f"card was {m['Maya']['charged_amount']}.")
    check("Maya's backstop card absorbed exactly the shortfall the other two couldn't cover",
          0 < m["Maya"]["charged_amount"] < BACKSTOP_CAP)
    check("the merchant received the full cart", auth.captured == CART.amount,
    ...
    for e in receipt["entries"]:
        ...
        say(f"    {e['name']:<8} charged={e['charged_amount']:<6} outcome={e['outcome']:<9} "
            f"hash={e['hash'][:12]}… chained-from-prev={linked}")
        prev = e["hash"]
    check("the receipt chain is unbroken from the genesis hash to chain_head",
    ...
    for k in ("authorization_conserved", "no_pooled_funds", "settlement_conserved"):
        say(f"  {k}: {report[k]}")
    check("authorization_conserved — no unit of authorized headroom invented or lost",
          report["authorization_conserved"])
    check("no_pooled_funds — no agent's headroom ever exceeds what it started with",
          report["no_pooled_funds"])
    check("settlement_conserved — every captured unit lands at exactly one merchant",
...
async def run_the_cancel_contrast() -> None:
    ...
    say(f"decision: {final.get('decision_note')!r}")
    say(f"verify_payment -> {status}")
    check("the group cancels — not partial, not committed", final["status"] == "aborted",
          final["status"])
    check("nobody was ever charged, including the two who already approved",
          auth.captured == 0, str(auth.captured))
...
async def run_agent_cannot_pay_agent() -> None:
    ...
    beat("Soham calls payments.pay(AgentId('Arsh'), Money(amount=500), ref) — no error, no "
    ...
    say(f"where the 500 actually went: conservation_report()['merchants'] = "
        f"{report['merchants']}")
    check("Arsh's own headroom never moved — she was not paid", after == before,
          f"{after} == {before}")
    check("the 500 was captured against a merchant record named 'Arsh', not credited to "
          "agent Arsh's wallet", report["merchants"].get("Arsh") == 500)
    check("no_pooled_funds holds — Arsh was not credited by Soham's payment",
          report["no_pooled_funds"])
...
async def run_prepaid_credits_contrast() -> None:
    ...
    check("prepaid_credits cannot express a group purchase — no such method exists",
    ...
    shared: dict[AgentId, int] = {
        AgentId("Soham"): 0, AgentId("Arsh"): 6200, AgentId("Dev"): 6200, AgentId("Priya"): 6200,
    }
    ...
    check("Soham — a coordinator, not a merchant — was credited by three other agents' "
          "payments", pooled == 18600, f"{pooled} == 18600")

    receipt = await soham_pc.pay(MERCHANT, Money(amount=18600), PaymentRef("pool-forward"))
    say(f"Soham then forwards the pool: pay({receipt.payee}, 18600) — no cap, no consent "
    ...
    rows = [
        ...
        ("who holds funds before the merchant is paid",
         "the coordinator's own simulator balance (18600 credits)", "nobody — cards only"),
        ...
        ("per-principal consent enforced by", "nothing — it's one balance transfer",
...
async def run_live_scene(base_url: str, token: str) -> None:
    ...
    say(
        ...
        + ("the engine's own Prava simulator — auto-approval can stand in for the "
           ...
           if mock else
           "a REAL Prava key. Approval URLs are Prava's own hosted ceremony. Nothing "
    ...
    try:
        ...
        check("pay_group() completes against the live engine", False,
    ...
    if mock:
        check("group committed with auto-approval standing in for the passkey tap",
              group.status == "committed", group.status)
        status = await payments.verify_payment(ref)
        check("verify_payment is CONFIRMED", status is PaymentStatus.CONFIRMED)
        report = payments.conservation_report()
    ...
    check("the plugin refuses to (and cannot) approve a real mandate", approved is False,
          str(approved))

    status = await payments.verify_payment(ref)
    say(f"verify_payment -> {status}")
    check("verify_payment is PENDING — waiting on four humans, not faked as CONFIRMED",
          status is PaymentStatus.PENDING, str(status))
    check("nothing was captured", auth.captured == 0, str(auth.captured))

    report = payments.conservation_report()
    say(f"conservation_report: authorization_conserved={report['authorization_conserved']} "
    ...
    check("verify_payment is REFUNDED — cancelled for real, over HTTP, no card ever touched",
          final_status is PaymentStatus.REFUNDED, str(final_status))

    say("")
    say("Acts 4-5 of the simulated scene (the decline -> backstop -> commit cascade, and "
