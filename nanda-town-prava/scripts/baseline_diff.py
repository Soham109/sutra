...
async def main() -> int:
    ...
    b = [ln for ln in bench_yaml.read_text().splitlines()
    ...
    rule("the traces")
    print(f"  {base_trace}: {len(base_trace.read_bytes())} bytes, "
          f"{len(base_trace.read_text().splitlines())} events")
    print(f"  {prava_trace}: {len(prava_trace.read_bytes())} bytes, "
    ...
    rows = [
        ("value moved between agents in the simulator",
         base["value_credited_to_agents"], prava["value_credited_to_agents"]),
        ...
        ("value debited from agents",
         base["value_debited_from_agents"], prava["value_debited_from_agents"]),
        ("credits still pooled inside the simulator",
         base["final_total_in_simulator"], prava["final_total_in_simulator"]),
        ("value that left a real card",
         base["value_that_left_a_card"], prava["value_that_left_a_card"]),
        ("value that reached a merchant outside",
         base["value_that_reached_a_merchant"], prava["value_that_reached_a_merchant"]),
        ("distinct merchants paid",
         base["merchants_paid"], prava["merchants_paid"]),
        ("payments executed",
         base["receipts"], prava["receipts"]),
    ]
    ...
    assert_("both traces pass the same upstream validators identically", same and all(
        p for p, _, _ in base_v))
    assert_("prepaid_credits conserves value INSIDE the simulator",
    ...
    assert_("prepaid_credits moved value between agents",
            base["value_credited_to_agents"] > 0, str(base["value_credited_to_agents"]))
    assert_("prava_mandates credited NO agent from another agent's payment",
            prava["agents_ending_richer_than_they_started"] == 0,
            str(prava["agents_ending_richer_than_they_started"]))
    assert_("prava_mandates moved every unit out of the simulator",
    ...
    assert_("prepaid_credits moved nothing to any merchant",
            base["value_that_reached_a_merchant"] == 0)
    assert_("prava_mandates leaves no authorization on hold",
            prava["conservation_report"]["outstanding"] == 0)
    assert_("prava_mandates: authorization_conserved",
            prava["conservation_report"]["authorization_conserved"])
    assert_("prava_mandates: no_pooled_funds",
            prava["conservation_report"]["no_pooled_funds"])
    assert_("prava_mandates: settlement_conserved",
            prava["conservation_report"]["settlement_conserved"])
    assert_("prava_mandates: headroom_consistent",
            prava["conservation_report"]["headroom_consistent"])
