...
class PravaMandates:
    ...
    def __init__(
        ...
        *,
        ...
        resolved_timeout = timeout if timeout is not None else _env_float("NANDA_PRAVA_TIMEOUT", 10.0)

        if engine is not None:
            self._bundle = _Bundle(transport=engine)
        else:
            self._bundle = _bundle_for(
                self._mode, resolved_base, resolved_token, resolved_timeout
            )
        ...
        if auto_approve is None:
            auto_approve = (
                self._mode == MODE_SIMULATED
                or _env_flag("NANDA_PRAVA_AUTO_APPROVE_MOCK")
            )
    ...
    async def pay(self, to: AgentId, amount: Money, ref: PaymentRef) -> Receipt:
        ...
        **not** credited, because the money is not in the simulator — it is
        ...
        if principals:
            ...
            return self._receipts[ref] if ref in self._receipts else self._receipt(
                ref, to, amount, group.status
            )
    ...
    def _receipt(
        self, ref: PaymentRef, to: AgentId, amount: Money, status: str
    ) -> Receipt:
