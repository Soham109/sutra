...
**Updated 2026-08-03.** The largest gap this document declared — §6.1 "no real
card was charged, anywhere" and §6.5 "no human ever approved a mandate in any of
these runs" — has since closed on the sandbox rail: two humans approved by
passkey, two mandates charged, one signed receipt. §9 records it with the
artifacts to check independently. §6 is left exactly as written; it was true when
written, and a dated record is worth more than a tidy one.

The short version of this document is [`NANDA.md`](NANDA.md). This file is the
long form, kept at this path because
[`projnanda/nandatown#210`](https://github.com/projnanda/nandatown/pull/210)
links to it.
...
1. **[Superseded 2026-08-03 — see §9. The sandbox half of this is now done; the
   production half still stands.]** **No real card was charged. Anywhere.** The deployed engine now holds a real
...
5. **[Superseded 2026-08-03 — see §9.]** **No human ever approved a mandate in any of these runs.** The passkey
...
## 9. Human-approved sandbox charge — 2026-08-03

§6.1 and §6.5 named the same gap: the `approved → charging → charged`
transition needs a human passkey, and nothing in this package can press one. It
was the largest thing this document could not prove. On 2026-08-02 at 23:28 IST
(`2026-08-02T17:58:17.918Z`) two people pressed it.

This is the **sandbox** rail. `/api/health` reports `prava_adapter: sandbox`,
and the second half of §6.1 — sandbox is not production — is unchanged. What
closed is the human-approval step, not the production claim.

### 9.1 What happened

| | |
| --- | --- |
| Group | `gs_01KZ1SW0EXN2V3N4Y1V0K5E4H4` |
| Merchant | Sutra House (`sutra-agzdw2mf.myshopify.com`), IN |
| Policy | `all_of` — everyone approves or nobody pays |
| Rail | `prava_mandates` (the one rail where `charges: true`) |
| Payers | Soham, arsh_new |
| Cap each | ₹9,393.00 (`939300` minor units) |
| Charged each | ₹9,300.00 (`930000` minor units) |
| Charged total | ₹18,600.00 |
| Status | `committed` |

Mandates `mdt_01KZ1T0RWTRDXPMJZC7S1254PY` and `mdt_01KZ1SYZ8D6HSRSMZ4HB3JMNVW`;
charges `txn_01KZ1T0SN9ME998GWCCC68EXQ2` and `txn_01KZ1T0T4TD4ECEE7WRGSN2H7B`.
Both charges landed under their own cap, which is the invariant the rail exists
to enforce.

### 9.2 Check it without trusting this file

```bash
curl -s https://sutra-gmp.vercel.app/api/health | jq .receipt_public_key
...
Chain head `22e4d82239659079199be3b1d7e40ee5afa360a2105a07fecd26e570bb4f9d78`,
signed by `b71838a635e97a8f8104e95213bbf3b718f64d89c13d645a8ab6245ca1f8de94`,
which is the key `/api/health` serves live. Verified two ways: `gmp verify`
above, and a second verifier written against `node:crypto` alone that imports
nothing from this repository, so the chain and signature are not being checked
by the code that produced them. Both agree, and tampering with any single field
breaks the entry hash, the `prev_hash` link, and the signature together.

### 9.3 What this still does not prove

- **Sandbox is not production.** No production Prava key has been issued to this
  engine, and the application is open. Nothing here says a real settlement
  reached a real merchant bank account.
- **The engine's own record is what was verified.** The signature proves the
  engine attested these charges and that the attestation has not been altered.
  It is not independent confirmation from the card network. Prava confirmed the
  transaction out of band; that confirmation is an email, not a cryptographic
  artifact, and is not offered here as one.
- **Two payers, not four.** The four-principal decline-and-backstop path in §8.3
  remains a `town_scene.py` run against the simulator, unchanged by this.
- Every other item in §6 stands as written.
