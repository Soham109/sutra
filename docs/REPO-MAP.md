...
> **`STATUS.md` and `HANDOFF.md` have since been retired and deleted.** Their durable content moved to [`ENGINEERING-NOTES.md`](ENGINEERING-NOTES.md) (the invariants, the Prava traps, the file ownership map), [`RUNBOOK.md`](RUNBOOK.md) (operations) and [`HACKATHON.md`](HACKATHON.md) (the pre-existing work disclosure); current state now lives in [`../AUDIT.md`](../AUDIT.md) and the work queue in [`../TASKS.md`](../TASKS.md). The findings below are left exactly as written, because they are the record of *why* those two files were retired. Their file:line citations no longer resolve, by design.
