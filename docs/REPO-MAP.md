...
| `docs/` | Nine markdown files: `HACKATHON.md`, `COORDINATION.md`, `AGENT-MESH.md`, `NANDA-EVIDENCE.md`, `NANDA-REGISTRY.md`, `PRODUCT_ARCHITECTURE.md`, `PRODUCT_AND_MOBILE_ROADMAP.md`, `RUNBOOK.md`, `README.md`. 4,040 lines total (`wc -l docs/*.md`). This file is now a tenth. | — | 232K |
| `film/` | Video production assets for the submission video: `film/scenes` (140K of scripts/storyboard) and `film/build` (6.6G of rendered `frames/`, `sfx/`, `voice/` — build output, not source). Not code. | — | 6.7G (6.6G is `build/`) |
...
- **No balance, ledger, or wallet table exists anywhere in this schema.** STATUS.md §3 makes this claim in prose ("there is no balance anywhere in the schema, by design"); I independently confirm it by having read every `CREATE TABLE` in the engine — there genuinely is none.
...
Ran directly, this session, exactly as instructed (PowerShell for the engine suite — Git Bash does throw a config error and run zero tests, confirmed, and per instructions I did not "fix" that; I just used PowerShell as directed):
...
| `bill-auction.test.ts` | the shared-dish-became-an-auction regression (STATUS.md §4, fixed) |
...
| `cancel-authority.test.ts` | who may cancel a group with an account behind it (STATUS.md §4, fixed) |
...
| `crash-double-charge.test.ts` | crash-between-charge-and-bookkeeping (STATUS.md §4, fixed) |
...
| `location-phrase.test.ts` | lowercase/informal place names still geocoding (STATUS.md §4, fixed) |
...
| `placeholder-names.test.ts` | a model inventing "friend1"/"friend2" is rejected (STATUS.md-adjacent regression) |
...
| `plan-participant-privacy.test.ts` | the plan-participant IDOR (STATUS.md §4, fixed) |
...
Independently, `POST /v1/members/:id/accept` (`routes.ts:216-246`) enforces the mirror-image rule at read time: if the seat *does* have a `user_id`, only that account (or the `ENGINE_API_TOKEN` bearer) may call accept on it (`routes.ts:234-241`) — "anyone with the group link could record somebody else's agreement, and on this rail the output is a SIGNED RECEIPT" (`routes.ts:224-229`). A bare-name seat has no such gate because there is no account to gate against — the link itself *is* the credential (`routes.ts:231-233`), which is the entire pass-the-phone design STATUS.md §4b describes fixing (renaming `assertLinkedFriends` → `assertSeatable` after an over-tightened version rejected bare names outright).
...
1. **Test counts are stale in every doc that states one, and each is stale by a different amount** — evidence the suite keeps growing across sessions faster than any doc gets refreshed:
   ...
   None of these four documented numbers match the current tree, and they don't even agree with each other — `STATUS.md` alone contradicts itself between its header and its own §4b, three sections and (per the file's internal timestamps) several hours apart. This is not a one-off staleness; it is a pattern worth flagging to whoever writes the submission: **do not quote a test count from any existing doc without re-running the suite first.**
...
3. **`GroupStatus.draft` (`types.ts:123`) is declared in the type but assigned nowhere.** Not a contradiction of any doc I found (no doc claims `draft` is used), but worth recording here since it's exactly the kind of fact a documentation-writing agent would otherwise have to rediscover by grepping — I already did the grep (`grep -rn "'draft'" engine/src/`, one hit, the declaration).
...
5. **`catalog/sources.ts` and `catalog/resolver.ts` disagree, harmlessly, about what "a source" means for WooCommerce.** No doc I read claims WooCommerce is a federated search source the way Shopify is, but the natural reading of "Shopify/WooCommerce storefronts" as a paired external dependency (as phrased in this task's own brief) would suggest parity. It is not symmetric: Shopify has a `CatalogSource` class searchable independent of any URL (`ShopifySource`, `catalog/sources.ts:16-96`); WooCommerce support exists only as one strategy inside the single-URL resolver (`fromWooCommerce`, `catalog/resolver.ts:383-433`) with no keyword-search equivalent. Someone writing user-facing copy from this map should say "paste a WooCommerce product link" and avoid implying WooCommerce stores are keyword-searchable the way the default Shopify shelf is.
