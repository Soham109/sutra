...
## Recommended main demo — two devices, real Prava sandbox, no Shopify setup

Use this as the payment proof. Shopify is an optional adapter-mapping appendix, not a prerequisite
for the main demo.
...
Sandbox-only card data is in [`../HANDOFF.md`](../HANDOFF.md). Never show the PAN, CVV, OTP,
...
### Optional Shopify development-store appendix

Skip this entire section for the recommended two-device Prava demo. Use it only if a judge asks
to see how completed test outcomes map into a merchant order record.
